...
with t1 as (
	select 
		...
		as address,
		bbl
	from hpd_contacts_dec_18 c
	left join hpd_registrations_grouped_by_bbl_dec_18 r
...
evictions_by_bbl_with_data as (
	select
		...
		coalesce(r.unitsstab2017,0) unitsstab2017
	...
	left join pluto_18v1 p using(bbl)
	left join rentstab_summary r on r.ucbbl = e.bbl
),
...
hpd_associated_bbls_with_evictions as (
	select 
		a.associated_bbls,
		sum(evictions) as total_evictions_18,
		sum(filings) as total_filings_1315,
		sum(unitsres) as total_unitsres,
		sum(unitsstab2017) as total_rs_units_17
